...
namespace VirtualClient.Actions
{
    ...
    public class WrkExecutorTests
    {
        ...
        public void GetWrkVersionReturnsCorrectVersion()
        {
            ...
            {
                Assert.AreEqual(command, "sudo");
                Assert.AreEqual(arguments, $"bash {executor.Combine(executor.PackageDirectory, WrkExecutor.WrkRunShell)} --version");
                this.memoryProcess.StandardOutput = new ConcurrentBuffer(new StringBuilder(wrkOutput));
                return this.memoryProcess;
            };
        ...
        public void GetWrkVersion_ThrowsException_WhenVersionCannotBeParsed()
        {
            ...
            // Process returns invalid output that doesn't contain version info
            this.mockFixture.ProcessManager.OnCreateProcess = (command, arguments, workingDir) =>
            {
                Assert.AreEqual(command, "sudo");
                Assert.IsTrue(arguments.Contains("--version"));
                this.memoryProcess.StandardOutput = new ConcurrentBuffer(new StringBuilder("Invalid output without version"));
                return this.memoryProcess;
            };
