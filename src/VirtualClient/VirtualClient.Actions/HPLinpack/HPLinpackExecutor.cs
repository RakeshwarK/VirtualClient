        ...
        private void CaptureMetrics(string results, string commandArguments, DateTime startTime, DateTime endTime, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            foreach (Metric result in metrics)
            {
                this.Logger.LogMetric(
                    ...
                    result.Unit,
                    null,
                    $"[ -N {this.ProblemSizeN} -NB {this.BlockSizeNB} -P {this.ProcessRows} -Q {this.ProcessColumns} {this.commandArguments} --perfLibrary={this.PerformanceLibrary} --perfLibraryVersion={this.PerformanceLibraryVersion} ]",
                    this.Tags,
                    telemetryContext,
                    result.Relativity,
