...
namespace VirtualClient.Actions
{
    ...
    public class BombardierExecutor : VirtualClientMultiRoleComponent
    {
        ...
        {
            // This retry is for the whole execution flow right from waiting for server to be online
            ...
            // This retry is for the individual workload execution
        ...
        protected async Task ExecuteWorkloadAsync(string commandArguments, string workingDir, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            string bombardierPath = this.Platform == PlatformID.Unix
                ? this.Combine(this.PackageDirectory, "bombardier")
                : this.Combine(this.PackageDirectory, "bombardier.exe");
            ...
            try
            {
                await (this.ClientRetryPolicy ?? Policy.NoOpAsync()).ExecuteAsync(async () =>
                {
                    try
                    {
                        ...
                        WorkloadAffinitySupport.AddAffinityContext(relatedContext, this.BindToCores, this.CoreAffinity, affinityConfig);
        ...
        protected string GetBombardierVersion(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            string versionPattern = @"bombardier\s+version\s+(\d+\.\d+\.\d+)";
